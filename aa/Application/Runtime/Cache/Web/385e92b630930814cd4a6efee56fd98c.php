...
<!-- header end -->
		...
		<div class="pub_object">
			<div class="title">
				<div class="title_c">
					<h2><?php if($tit['Source'] == 1): ?>司法拍卖评估<?php elseif($tit['Source'] == 6): ?>出租尽调<?php else: ?>转让尽调<?php endif; ?>发布页面</h2></div>
			</div>
			<div class="main">
				<form action="<?php echo U('Web/Assetsrent/domic');?>" id="pub_Object" method="post" enctype="multipart/form-data">
				<div class="pub_report">
					<ul>
						<li>
							<b>项目名称：</b>
							<i><?php echo ($tit["Title"]); ?></i>
							<input type="hidden" name="AssetsID" value="<?php echo ($tit["AssetsID"]); ?>">
						...
						<li>
							<b><?php if($tit['Source'] == 1): ?>评估<?php else: ?>尽调<?php endif; ?>报告介绍：</b>
							<em><textarea class="form-control" name="ReportDesc"></textarea></em>
						</li>
						<li>
							<b><?php if($tit['Source'] == 1): ?>评估<?php else: ?>尽调<?php endif; ?>报告：</b>
							<div class="fl">
								<div class="dp-lig">
								<div class="dp-boxa">
										<a href="">
											<img src="//www.3qqq.com/static/images/common/up_01.png">
											<input class="up_file" type="file" name="orginpic" id="fload4" name="fload4" zzp="zip">
											<input class="up_file"  type="hidden" name="JDUrl"   value="" id="jdurl" >
										</a>
									</div>
								</div>
							</div>
							<div class="item">
								<p>1、请上传<?php if($tit['Source'] == 1): ?>评估<?php else: ?>尽调<?php endif; ?>报告，文件最大限制为499M。</p>
								<p>2、根据您的网络环境，该可能需要等待一段时间。</p>
								<p>3、请在本报告上传之后在进行其他操作。</p>
							</div>
						</li>
						<li>
							<b><?php if($tit['Source'] == 1): ?>评估<?php else: ?>尽调<?php endif; ?>报告部分公开展示：</b>
							<div class="fl">
								<div class="dp-lig">
									<div class="dp-boxa">
										<a href="">
							...
							<div class="item">
								<p>1、请上传<?php if($tit['Source'] == 1): ?>评估<?php else: ?>尽调<?php endif; ?>报告部分公开展示pdf文件，文件最大限制为499M。</p>
								<p>2、根据您的网络环境，该可能需要等待一段时间。</p>
								<p>3、请在本报告上传之后在进行其他操作。</p>
							</div>
						</li>
						...
						<li>
							<b>查看<?php if($tit['Source'] == 1): ?>评估<?php else: ?>尽调<?php endif; ?>报告费用：</b>
							<span>
						...
						<li>
							...
							<b>买断<?php if($tit['Source'] == 1): ?>评估<?php else: ?>尽调<?php endif; ?>报告费用：</b>
							<span>
							...
							<span class="price">购买费用为<strong><?php echo ($ggtc["CodeName"]); ?>元-<?php echo ($ggtc["CodeValue"]); ?>元一天</strong></span>
						</li>
						<div class="submit">
...
<script type="text/javascript">
	$(function(){
		...
		//var strint = $(".strendint").attr("strint"),endint=$(".strendint").attr("endint");
		var strint,endint;
