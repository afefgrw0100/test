		...
		<div class="main">
			<div class="pub_detail">
				...
				<div class="box1">
					...
					<div class="btn1">
						<?php if(($asset['AssetsStatue'] != 8)): if(($asset['AssetsStatue'] == 4) AND ($downurl != 88) AND ($asset['DueCreateMember'] != session('user.MemberId'))): ?><em class="price">¥<?php echo ($proinfo["ViewPrice"]); ?></em><a href="<?php echo U('User/Logic/buymic',array('type'=>'assetsrent',buyid =>$asset['AssetsID']));?>" class="buy">购买尽调报告</a><?php endif; ?>
							<?php if(($freenum['freeCountzz'] != -1) AND ($asset['AssetsStatue'] == 4) AND ($downurl != 88) AND ($asset['DueCreateMember'] != session('user.MemberId')) ): ?><button class="btn2" id="freeread" href="<?php echo U('Home/Lists/freeread',array('type'=>'assetsrent',buyid =>$asset['AssetsID']));?>">免费阅读（余<em id="freeCount"><?php echo ($freenum["freeCount"]); ?></em>次）</button><?php endif; ?>
							<?php if((($downurl == 88) OR ($asset['DueCreateMember'] == session('user.MemberId'))) AND ($asset['AssetsStatue'] == 4)): ?><a href="<?php echo U('User/Logic/downFile',array('type'=>'assetsrent',buyid =>$asset['AssetsID']));?>" class="buy" target="_blank">下载尽调报告</a>
								<if condition="($asset['AssetsStatue'] neq 7) AND ($asset['DueCreateMember'] neq session('user.MemberId'))">
									<span class="sale">
										<form method="get" action="<?php echo U('User/Common/deal',array('type'=>'assetsrent',buyid =>$asset['AssetsID']));?>" id="deal">
...
<div class="mask" id="mask">
	<div class="apply_content">
		<div class="agent-tan">
			<form action="<?php echo U('Home/Lists/dofinan',array('type'=>'assetsrent',buyid =>$asset['AssetsID']));?>" method="post" id="dddd">
