...
class CommonController extends UserbaseController{
    ...
    public function todo(){
        ...
        if(session("user.MemberType")==4){
            ...
            $debt_model = M("assets_rent");
            $debtinfo = $debt_model->field("
                tb_assets_rent.AssetsID,
                Title as title,
                Price as total,
                Source as source,
                ProcessModle as promodel,
                ...
                CellPhone as mobile,
                CreateTime,AssetsStatue,
                OptType")->join("LEFT JOIN tb_assetsacceptinfo_rent ON tb_assetsacceptinfo_rent.AssetsID = tb_assets_rent.AssetsID")->where($map)->order("AssetsID desc")->select();
//            $debt_model = M("debt");
//            $debtinfo = $debt_model->field("
//                tb_debt.DebtID,
//                Title as title,
...
//                OptType")->join("LEFT JOIN tb_packageacceptinfo ON tb_packageacceptinfo.PackageID = tb_package.PackageID")->where($map)->order("PackageID desc")->select();
            ...
            $debt_model = M("assets_rent");
            $debtinfo = $debt_model->field("
                tb_assets_rent.AssetsID as AssetsID,
                Title as title,
                Price as total,
                Source as source,
                ProcessModle as promodel,
    ...
    public function dealview(){
        ...
        switch($pid){
            ...
            case 14;
                ...
                $info = $order_model->field("id,AssetsID")->join("tb_assetsproject_rent ON tb_order.InfoNo=tb_assetsproject_rent.SubID")
                    // ->join("tb_memberbuyhistory ON tb_assetsproject.AssetsID = tb_memberbuyhistory.ProjectID")
                    ->where($map)->find();
                $map_his['ProjectID'] = $info['AssetsID'];
                $info_his = M("memberbuyhistory")->where($map_his)->find();
                $isget=array('AssetsIDrent'=>$info_his['ProjectID']);
            ...
            case 13;
                ...
                $isget=array('AssetsIDrent'=>$info['AssetsID']);
                $this->redirect("Home/lists/content",$isget);
...
// 进行分页数据查询 注意limit方法的参数要使用Page类的属性
    ...
    public function partnerproject(){
        ...
        switch($messagetype){
            case 0:
                $info_map['Source'] =array('in','2,3,4,5');
        ...
        if(is_array($user_in_array)){
            ...
            if($messagetype==0){
                ...
                $count1 = $assets_model->where($info_map)->count();
                $info_map['Source']=6;
                $count2 = $assetsrent_model->where($info_map)->count();
                $count = $count1+$count2;
                ...
                $info_map['Source']=array('in','2,3,4,5');
                $info02 = $assets_model->field("AssetsID,Title,Source,ProPrice,AssetsStatue,memberid,CreateTime")->where($info_map)->select();
                $info_map['Source']=6;
                $info01 = $assetsrent_model->field("AssetsID,Title,Source,ProPrice,AssetsStatue,memberid,CreateTime")->where($info_map)->select();
                $info=array_merge_recursive($info,$info02,$info01);
                $info = array_slice($info,$Page->firstRow,10);
            }else{
                $info = $assets_model->field("AssetsID,Title,Source,ProPrice,AssetsStatue,memberid,CreateTime")->where($info_map)->limit($Page->firstRow.','.$Page->listRows)->select();
            }
    ...
    public function deputelist(){
        ...
        $show       = $Page->show();// 分页显示输出
// 进行分页数据查询 注意limit方法的参数要使用Page类的属性
        $lists = $model_suf->where($map)->order("id desc")->limit($Page->firstRow.','.$Page->listRows)->select();
        // print_r($lists);
        $this->assign('list',$lists);// 赋值数据集
        $this->assign('page',$show);// 赋值分页输出
        $this->display("Member:depute");
    }
