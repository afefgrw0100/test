...
class PaymentController extends  MemberbaseController{
    ...
    public function index(){
        ...
        switch($codetype){
            ...
            case 1:
                ...
                if($type=="assetsrent"){
                    ...
                    $result =$order_model->field("id,sn,DealPrice,Title")->join("tb_assets_rent ON tb_order.InfoNo=tb_assets_rent.AssetsID")->where($map)->find();
                }
                if($type=="package"){
            ...
            case 2;
                ...
                if($type=="assetsrent"){
                    $result =$order_model->field("id,sn,DealPrice,tb_assetsproject_rent.SubName as Title")->join("tb_assetsproject_rent ON tb_order.InfoNo=tb_assetsproject_rent.SubID")->where($map)->find();
                }
                if($type=="package"){
            ...
            case 5:
                ...
                if($type=="assetsrent"){
                    ...
                    $result =$order_model->field("id,sn,DealPrice,ProductID,tb_assetsproject_rent.SubName as Title,tb_assetsproject_rent.AssetsID as pid")->join("tb_assetsproject_rent ON tb_order.InfoNo=tb_assetsproject_rent.SubID")->where($map)->find();
        ...
        $balance = $this->balancepayment($result);

        if($balance){
            header("Content-Type:text/html;charset=UTF-8");
            session('login_http_member',U("User/Common/ordermgt"));
            if(is_mobile()){

                redirect(U("User/Common/ordermgt"),2,"余额支付成功");
            }else{

                redirect(U("User/Member/index"),2,"余额支付成功");
            }
    ...
    public function weixinpay_qrcode(){
        ...
        $DealPrice = $result['DealPrice']-yue();
        $order=array(
            'body'=>$tit,
            'total_fee'=>$DealPrice*100,
        ...
        switch($codetype){
            ...
            case 1:
                ...
                if($type=="assetsrent"){
                    ...
                    $result =$order_model->field("id,sn,DealPrice,Title,UpdateTime")->join("tb_assets_rent ON tb_order.InfoNo=tb_assets_rent.AssetsID")->where($map)->find();
                }
                if($type=="package"){
            ...
            case 2;
                ...
                if($type=="assetsrent"){
                    $result =$order_model->field("id,sn,DealPrice,tb_assetsproject_rent.SubName as Title,tb_order.UpdateTime")->join("tb_assetsproject_rent ON tb_order.InfoNo=tb_assetsproject_rent.SubID")->where($map)->find();
                }
                if($type=="package"){
    ...
    private function balancepayment($result){
        ...
        if(!empty($result)){
            ...
            if($balance>$result['DealPrice']){
                //修改订单
                $data['balance'] = $result['DealPrice'];
                $data['OrderStatue'] =2;
                $data['UpdateTime'] =date("Y-m-d H:i:s");
                $map_order['id'] = $result['id'];
                M()->startTrans();
                $res = M("order")->where($map_order)->save($data);

                //修改余额
                $free_where['MemberId'] =$memberid;
                $memberaccount =  M("memberaccount")->where($free_where)->find();
                $memberaccountData['Balance']=$memberaccount['Balance']-$result['DealPrice'];
                M("memberaccount")->where($free_where)->save($memberaccountData);

                //修改变动记录
                $cdata['SN'] = date("YmdHis").func_getRandString(4);
                $cdata['MemberId'] = $memberid;
                $cdata['PreIntegral'] = $memberaccount['Balance'];
                $cdata['NxtIntegral'] = $memberaccount['Balance']-$result['DealPrice'];
                $cdata['OffSetValue'] = -$result['DealPrice'];
                $cdata['Remark'] = "余额支付";
