...
class AssetsController extends MemberbaseController {
...
//        };
            ...
            if($assets_model->create()){
              // print_r($assets_model->create());exit;
                ...
                if(empty(I('post.cid'))){
                    ...
                    if($result){
                        if(session("user.MemberType")==4){
                            $this->redirect("Web/Index/mic",array('AssetsID'=>$result));
                        }else{
                            $this->userorder($result,"assets");
                        }
    ...
    public function domic(){
        if(IF_POST){
            ...
            $map['Usestat'] =1;
            $res = $model->table("tb_assetsduedili")->where($map)->find();
            if($model->table("tb_assetsduedili")->validate($ultdili)->create()){
                $model->table("tb_assetsduedili")->Images = basename($model->Images[0])."@".basename($model->Images[1])."@".basename($model->Images[2]);
                $model->table("tb_assetsduedili")->DueCreateMember = session("user.MemberId");
                $model->table("tb_assetsduedili")->DueCreateTime =date("Y-m-d H:i:s");
                $model->table("tb_assetsduedili")->DescPdf =explode("?",basename($model->table("tb_assetsduedili")->DescPdf))[0];
                if(!empty($res)){
                    $result =  $model->table("tb_assetsduedili")->where($map)->save();
                }else{
                    $result =  $model->table("tb_assetsduedili")->add();
                }


                if($result){
                    if($model->table("tb_assetsproject")->validate($ultpro)->create()){
                        $model->table("tb_assetsproject")->KXUrl= explode("?",basename($model->table("tb_assetsproject")->JDUrl))[0];
                        $model->table("tb_assetsproject")->JDUrl= "JDUrl";
                        if(!empty($res)){
                            unset($map['Usestat']);
                            $result= $model->table("tb_assetsproject")->where($map)->save();
                        }else{
                            $result= $model->table("tb_assetsproject")->add();
                        }
                        ...
                        if($result){
                            ...
                            M("assets")->where($AssetsID)->setField($AssetsStatue);
                            $model->commit();
