...
class ListsController extends HomebaseController{
    ...
    public function lists(){
        ...
        $areacity['pid'] = array("between","1,34");
...
//                    $data['DealInt']= $info['ViewPrice']*$orging['CodeValue']*$codeval['CodeValue'];
    ...
    public function mobilecity(){
        ...
        $area = M("area");
        $areamap['pid'] = '0';
        $areainfo =  $area->field("id,name,pid")->where($areamap)->order('id ASC')->select();
...
//        $map['_complex'] = $wheremap;
        ...
        if(!empty($card['MemberId'])){
            $shfe = shfeng( $map["tb_assets.AssetsID"],$card['MemberId'],1,"实物分享");
            //echo $shfe;
        }
