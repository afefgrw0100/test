    ...
    **/
    ...
    public function getParameters(){
        ...
        if (!isset($_GET['code'])) {
            ...
            // 返回的url
            $redirect_uri=U('User/Apipay/pay','','',true);
            ...
            $map['sn'] = $out_trade_no;
            $descinfo= M('order')->field("id,sn,ProductDesc,DealPrice")->where($map)->find();
            $DealPrice = $descinfo['DealPrice']-yue();
            $order=array(
                'body'=>$descinfo['ProductDesc'],// 商品描述（需要根据自己的业务修改）
                'total_fee'=>$DealPrice*100,// 订单金额  以(分)为单位（需要根据自己的业务修改）
                'out_trade_no'=>$out_trade_no,// 订单号（需要根据自己的业务修改）
                'product_id'=>$descinfo['id'],// 商品id（需要根据自己的业务修改）
