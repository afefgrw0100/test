			...
			<div class="info1">
				...
				<div class="con">
					<?php if($asset['PayModes'] == 0): ?><ul>
						...
						<li>
							<span><b><?php if($source == 6): ?>月租价格<?php else: ?>出售价格<?php endif; ?>： </b><em><i class="numtenhousand"><?php echo ($asset["proprice"]); ?></i>元</em></span>
				...
				<div class="base">
					<div class="share">
						<?php if(($asset['AssetsStatue'] != 8) AND (($source != 1) || !comper_time($asset['AuctionEnds']))): if(($asset['AssetsStatue'] == 4) AND ($downurl != 88) AND ($asset['DueCreateMember'] != session('user.MemberId'))): ?><em class="buy_btn"><a href="<?php echo U('Home/Lists/pdfview',array('fileurl'=>urlencode(imgpublic($asset['DescPdf'])),'title'=>$asset['title']));?>" >预览部分报告</a></em>
							...
							<?php if(($freenum['freeCountzz'] != -1) AND ($asset['AssetsStatue'] == 4) AND ($downurl != 88) AND ($asset['DueCreateMember'] != session('user.MemberId')) ): ?><em class="buy_btn"><a href="<?php echo U('Home/Lists/freeread',array('type'=>'assets',buyid =>$asset['AssetsID']));?>" >免费阅读（余<em id="freeCount"><?php echo ($freenum["freeCount"]); ?></em>次）</a></em><?php endif; ?>
							<?php if((($downurl == 88) OR ($asset['DueCreateMember'] == session('user.MemberId'))) AND ($asset['AssetsStatue'] == 4)): ?><em class="buy_btn"><a href="<?php echo U('User/Logic/downFile',array('type'=>'assets',buyid =>$asset['AssetsID'],'viewfile'=>'viewfile'));?>" >查看尽调</a></em><?php endif; endif; ?>
					</div>
				</div>
				<div class="base">
		...
		<script type="text/javascript" charset="utf-8">
			...
			if(unconfirmBtn!=null){
				document.getElementById("confirmBtn").addEventListener('tap', function() {
					var memberid= "<?php echo ($_SESSION['user']['MemberId']); ?>";
					if(memberid==""){
						window.location.href="<?php echo U('User/Index/index');?>";
						return false;
					}
					var a="100";
					var btnArray = ['取消', '支付'];
					var yue = Number("<?php echo yue();?>");
					var ViewPrice =Number("<?php echo ($proinfo["ViewPrice"]); ?>");
					var Actualpayment = 0;
					if(yue>ViewPrice){
						Actualpayment = 0;
						console.log(yue);
					}else {
						Actualpayment =ViewPrice - yue;
						console.log(Actualpayment);
					}
					mui.confirm( '在线支付：'+'¥'+Actualpayment,'可用余额：'+'¥<?php echo yue();?>', btnArray, function(e) {
						if (e.index == 1) {
							window.location.href="<?php echo U('User/Logic/buymic',array('type'=>'assets',buyid =>$asset['AssetsID']));?>";
						} else {
							console.log('取消支付')
						}
					})
