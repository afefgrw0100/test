...
class IndexController extends HomebaseController {
    ...
    public function index(){
        ...
        //新闻列表
        $map_train['type'] = 11;
        $list =  M()->table("sys_articlescrap")->field("id,title,createtime")->where($map_train)->order(array( 'no' => 'asc','createtime' => 'desc'))->limit(10)->select();
        $this->assign('artlist',$list);// 赋值数据集

        //特金学院
        $map_train['type'] = 20;
        $list_tj =  M()->table("sys_articlescrap")->field("id,title,createtime")->where($map_train)->order(array( 'no' => 'asc','createtime' => 'desc'))->limit(10)->select();
...
//        $this->assign("cpro",$cpro);
        ...
        $substation=  M()->table("sys_area_photo")->field("area_code,titleImg,title")->where($substation_map)->select();
...
//        $info_assets_auc=$assets->where($maps)->order("AssetsID DESC")->order('AssetsID  desc')->limit(12)->select();
       // print_r($info_assets_auc);
        //出租
        $maps['Source'] = 6;
        $info_assets_Le=$assets->where($maps)->order("AssetsID DESC")->order('AssetsID  desc')->limit(12)->select();
        //print_r($info_assets);
...
//        $info_packs_img =$packs->field("PackageID,titimg,Title,ViewCount,BuyCount,ProcessModle as ccint,Total as aaint,AssetsStatue")->join("LEFT JOIN tb_packagedata ON tb_package.PackageID = tb_packagedata.ID")->where($map)->order(array( 'BuyCount' => 'desc','ViewCount' => 'asc'))->limit(8)->select();
      ...
      //  $info_img = array_merge_recursive($info_assets_img,$info_debt_img,$info_packs_img);
        $info_img = array_merge_recursive($info_assets_img);
        //print_r($info_img);
        $arrsort = null;
...
//        }
    ...
    public function Auction(){
       ...
       // $city ="272";
        $city_map['code'] = $city;
        $cityname = M("area")->field("id,name")->where($city_map)->find();
        $city = $cityname['id'];
       ...
       // print_r($user_in);exit;
        if(!is_null($user_in_array)){
           // $info_map['memberid'] =array("in",$user_in_array);
            ...
            //出租
            $info_map['Source'] =6;
            $info_b = $assets_model->field("AssetsID,titimg,Title,Source,Price,ProPrice,AssetsStatue,AssetsType,memberid,CreateTime,City")->where($info_map)->order('AssetsID  desc')->limit(8)->select();
